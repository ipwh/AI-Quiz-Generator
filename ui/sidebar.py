...
# ✅ 老師輸入自己 Key → 覆蓋
# ✅ 進階 expander：OpenAI / xAI / Azure / 自訂
# =========================================================
...
def render_sidebar() -> dict:
    ...
    # =========================================================
    # 區塊 B：進階 expander（IT / 熟手）
    # =========================================================
    advanced_cfg: dict | None = None  # None = 用 DeepSeek 預設

    with st.sidebar.expander("⚙️ 進階：選擇其他 LLM", expanded=False):
        st.caption("一般老師毋須設定。IT 或進階用戶可在此切換 LLM。")
        ...
        if adv_preset == "DeepSeek":
            ...
            adv_model   = st.text_input("Model", key="adv_model",
            ...
            az_api_version = st.text_input("API version", value="2024-02-15-preview",
                                            key="adv_az_ver")
    ...
    def api_config() -> dict:
        ...
        if advanced_cfg:
            return advanced_cfg
        # 否則用 DeepSeek（內置/老師 key + 自動 model）
        return {
            "type": "openai_compat",
    ...
    if st.sidebar.button("🧪 一鍵測試 API（回覆 OK）", key="btn_ping_api"):
        if ping_llm is None:
            st.sidebar.warning("⚠️ llm_service 未提供 ping_llm()。")
        elif not can_call_ai(cfg_test):
            st.sidebar.error("❌ 請先填妥 API Key 或聯絡 IT 設定校內預設 Key。")
        else:
            with st.sidebar.spinner("正在測試連線…"):
                ...
                if r.get("ok"):
                    ...
                    st.sidebar.error("❌ 失敗：請檢查 Key / 服務狀態")
    ...
    st.sidebar.caption("數學／物理／化學建議開啟 Vision 模式。")
    ...
    if ocr_mode == "🤖 LLM Vision 讀圖（圖表/方程式/手寫，最準）":
        vision_pdf_max_pages = st.sidebar.slider(
            "Vision PDF 最多讀取頁數",
            min_value=1, max_value=10, value=3, key="vision_pdf_max_pages",
        )
    st.sidebar.info("💡 DeepSeek 不支援 Vision，請在進階切換至 Grok / GPT-4o。")
    ...
    if SUBJECT_GROUPS:
        ...
        default_idx = options.index("中國語文") if "中國語文" in options else 1
        sel = st.sidebar.selectbox("科目", options, index=default_idx, key="subject_grouped")
        while sel.startswith("—") and sel.endswith("—"):
            idx = options.index(sel) + 1
            sel = options[idx] if idx < len(options) and not options[idx].startswith("—") else "中國語文"
            break
    ...
    return {
        ...
        # debug
        "model": cfg_test.get("model", deepseek_model),
        "base_url": cfg_test.get("base_url", deepseek_base_url),
