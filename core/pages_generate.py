...
try:
    from services.llm_service import generate_questions, llm_ocr_extract_text
except Exception:
    from llm_service import generate_questions, llm_ocr_extract_text
...
def render_generate_tab(ctx: dict):
    """生成新題目頁面（與 pages_import.py 對稱的流程）。"""
    ...
    if file:
        # Sidebar radio uses key="ocr_mode" and slider key="vision_pdf_max_pages" in sidebar.py
    ...
    if not paras:
        st.info("提示：教材沒有分段文字（或目前是圖片/掃描件）。如屬掃描件，建議切換到『本地 OCR』或『LLM Vision』。")
    else:
        st.caption("勾選後，AI 會優先根據『重點段落』出題。")

        # Build a fresh set each run (checkboxes are stateful via keys)
        marked = set()
    ...
    if not can_call_ai(cfg):
        st.warning("⚠️ 請先在左側填妥 AI API Key／Base URL／Model（Azure 要 Endpoint + Deployment），並測試連線。")
    ...
    if st.button("🪄 生成題目", disabled=disabled_generate, key="btn_generate_questions"):
        with st.spinner("🧠 AI 出題中…"):
            ...
            # If we have images (Vision mode or scanned PDF render), call multi-modal path;
    ...
    if st.session_state.get("generated_items"):
        ...
        # Persist edits back to domain items
