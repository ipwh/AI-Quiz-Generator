...
try:
    ...
    _HAS_EXTRACT_IMAGES = True
except Exception:
    extract_images_for_llm_ocr = None
    _HAS_EXTRACT_IMAGES = False
...
st.caption("UI版本：UI-REDESIGN 2026-04-19 v2 ✅（見到呢句＝你已成功換到新版）")
...
with tab_generate:
    st.markdown("## ① 上載教材")
    st.caption("支援 PDF/DOCX/TXT/PPTX/XLSX/PNG/JPG。若是掃描/截圖，可開啟 LLM 讀圖 OCR（較慢）。")
    ...
    enable_llm_ocr = st.checkbox(
        ...
        value=False,
        help="當抽取到的文字太少時，會把圖片/掃描PDF前幾頁交給 Grok 讀圖抽字，再用該文字出題。",
    )
    llm_ocr_pdf_pages = st.selectbox("LLM OCR PDF頁數（只取前幾頁）", [1, 2, 3, 4, 5], index=2)

    if enable_llm_ocr and (not _HAS_LLM_OCR or not _HAS_EXTRACT_IMAGES):
    ...
    if files:
        ...
        if enable_llm_ocr and _HAS_LLM_OCR and _HAS_EXTRACT_IMAGES and can_call_ai(cfg) and len(raw_text.strip()) < 200:
            cfg2 = cfg
...
" + raw_text).strip()
    ...
    if st.button("🪄 生成題目", disabled=not (can_call_ai(cfg) and bool(raw_text.strip())), key="btn_generate"):
        try:
            ...
            # Bump cache when prompt logic changes
            PROMPT_VERSION = "llmocr_fmtquota_v1"
            cache = load_cache()
