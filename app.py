...
# LLM OCR (optional): keep app stable even if functions are not present yet
try:
    from services.llm_service import xai_pick_vision_model, llm_ocr_extract_text
    HAS_LLM_OCR = True
except Exception:
    xai_pick_vision_model = None
    llm_ocr_extract_text = None
    HAS_LLM_OCR = False
...
# Image-to-dataURL for LLM OCR (optional)
try:
    from extractors.extract import extract_images_for_llm_ocr
    HAS_EXTRACT_IMAGES = True
except Exception:
    extract_images_for_llm_ocr = None
    HAS_EXTRACT_IMAGES = False
...
# -------------------------
# Anti-copy-break constants
# -------------------------
NL = chr(10)          # newline
DNL = chr(10) * 2     # double newline
...
def build_text_with_highlights(raw_text: str, marked_idx: set, limit: int):
    ...
    out_parts = []
    if highlights:
        out_parts.append("【重點段落（老師標記）】")
        out_parts.append(DNL.join(highlights))
    out_parts.append("【其餘教材】")
    out_parts.append(DNL.join(others))

    out = DNL.join(out_parts)
    return out[:limit]
...
def export_and_share_panel(selected_df: pd.DataFrame, subject_name: str, prefix: str):
    ...
    with c2:
        ...
        st.caption("用途：Wayground/工作紙（只包含已勾選題目）")

    st.markdown("### 📧 一鍵電郵分享匯出檔（Google Drive）")
    st.caption("會先上載到你的 Google Drive，然後分享並寄出通知電郵。")
...
with st.expander("🧭 使用流程（建議）", expanded=True):
    lines = []
    lines.append("**🪄 生成新題目（推薦）**")
    lines.append("1. 左側完成：Google 登入（可選）＋設定 AI API")
    lines.append("2. 選科目、難度、題目數目")
    lines.append("3. 上載教材 →（可選）標記重點段落")
    lines.append("4. 按「生成題目」→ 在表格內微調題幹/選項/答案")
    lines.append("5. 勾選要匯出的題目 → 匯出 Kahoot/Wayground、建立 Google Form、或用電郵分享")
    lines.append("")
    lines.append("**📄 匯入現有題目**")
    lines.append("1. 上載/貼上題目內容 →（可選）啟用 AI 協助整理")
    lines.append("2. 按「整理並轉換」→ 在表格內校對答案")
    lines.append("3. 匯出 / 建 Google Form / 電郵分享")
    st.markdown(NL.join(lines))
...
with tab_generate:
    st.markdown("## ① 上載教材")
    st.caption("支援 PDF/DOCX/TXT/PPTX/XLSX/PNG/JPG。掃描/截圖可選擇啟用 LLM 讀圖 OCR（較慢）。")

    cfg = api_config()

    enable_llm_ocr = st.checkbox(
        "🖼️ 啟用 LLM 讀圖 OCR（掃描/截圖/圖表/幾何較準，但較慢）",
        value=False,
        help="抽取文字太少時，會把圖片/掃描PDF前幾頁交給 Grok 讀圖抽字，再用該文字出題。",
    )
    ...
    if files:
        ...
        # LLM OCR fallback（cfg 已定義，raw_text 已抽取後才跑）
        if enable_llm_ocr and HAS_LLM_OCR and HAS_EXTRACT_IMAGES and can_call_ai(cfg) and len(raw_text.strip()) < 200:
            cfg2 = cfg
    ...
    if st.button("🪄 生成題目", disabled=not (can_call_ai(cfg) and bool(raw_text.strip())), key="btn_generate"):
        try:
            ...
            PROMPT_VERSION = "llmocr_fmtquota_v3"
            cache = load_cache()
