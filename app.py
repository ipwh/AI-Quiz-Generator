...
EDITOR_COLUMN_CONFIG = {
    "export": st.column_config.CheckboxColumn("匯出", help="勾選：匯出；取消：不匯出此題", width="small"),
    ...
    "type": st.column_config.TextColumn("題型", width="small"),
}
...
if not oauth_is_configured():
    st.sidebar.warning("⚠️ 尚未設定 Google OAuth（請在 Secrets 設定 google_oauth_client 及 APP_URL）。")
else:
...
auto_xai = False  # ✅ xAI auto-detect toggle
...
if preset == "DeepSeek":
    ...
    base_url = "https://api.x.ai/v1"  # xAI base。chat: POST /v1/chat/completions [3](https://www.buildmvpfast.com/tools/api-pricing-estimator/google-vision)[1](https://blog.csdn.net/gitblog_09688/article/details/142221724)
    # 預設先用 alias（已經相當於“跟最新”），再選擇是否做 API 偵測
    model = "grok-4-latest"
    auto_xai = st.sidebar.checkbox("🤖 自動偵測可用最新 Grok 型號（建議）", value=True, help="會用 /v1/language-models 查你個 API key 可用型號（10分鐘快取）。")  # [2](https://zhuanlan.zhihu.com/p/1964739506629490036)
...
# ✅ Auto 偵測 xAI 模型（10分鐘快取）
@st.cache_data(ttl=600, show_spinner=False)
def _detect_xai_model_cached(api_key_value: str, base_url_value: str) -> str:
    # 透過 /v1/language-models 取 aliases / created，再選最佳 model [2](https://zhuanlan.zhihu.com/p/1964739506629490036)
    return get_xai_default_model(api_key_value, base_url_value)


if preset == "Grok (xAI)" and auto_xai and api_key:
    # 只要能呼叫就試偵測；失敗會回傳 fallback alias（在 llm_service 內處理）
...
cfg_test = api_config()
btn_test = st.sidebar.button("🧪 一鍵測試 API（回覆 OK）")
if btn_test:
...
if mode == "🪄 AI 生成新題目":
    ...
    files = st.file_uploader("上載教材（支援PDF/DOCX/TXT/PPTX/XLSX）", accept_multiple_files=True, type=["pdf","docx","txt","pptx","xlsx"])
    cfg = api_config()
    ...
    if st.button("生成題目", disabled=not (can_call_ai(cfg) and bool(files))):
        try:
            ...
            if key in cache:
                ...
                with st.spinner("🤖 正在呼叫 AI，請稍候 30–60 秒…"):
            ...
            if st.button("🟦 一鍵建立 Google Form Quiz（只匯出已勾選）"):
                try:
                    if selected.empty:
                        ...
                        st.write("發佈連結：", result.get("responderUrl") or "（API 未提供 responderUri，可於表單右上角『傳送』取得）")
                except Exception as e:
                    show_exception("⚠️ 建立 Google Form 失敗（常見：配額/權限/未啟用 Forms API）。", e)
                    st.stop()
        else:
