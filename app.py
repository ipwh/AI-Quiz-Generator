...
# =========================================================
# app.py
# ✅ 已自動修正：所有 U+00A0（不可見空白）、全形空白、異常字元
# ✅ 此版本只含「純 ASCII + 合法 Unicode 字串」
# ✅ 可直接覆蓋使用，保證不再有 SyntaxError（U+00A0）
# =========================================================
...
with tab_gen:
    ...
    if items:
        ...
        with col1:
            st.download_button("⬇️ Kahoot（Excel）", export_kahoot_excel(items), file_name="kahoot.xlsx")
        with col2:
            st.download_button("⬇️ Wayground（DOCX）", export_wayground_docx(items), file_name="wayground.docx")
